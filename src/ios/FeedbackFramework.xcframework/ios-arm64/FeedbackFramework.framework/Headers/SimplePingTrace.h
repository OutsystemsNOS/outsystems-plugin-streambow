...
@protocol SimplePingTraceDelegate;

/*! Controls the IP address version used by SimplePing instances.
 */

typedef NS_ENUM(NSInteger, SimplePingTraceAddressStyle) {
    SimplePingTraceAddressStyleAny,          ///< Use the first IPv4 or IPv6 address found; the default.
    SimplePingTraceAddressStyleICMPv4,       ///< Use the first IPv4 address found.
    SimplePingTraceAddressStyleICMPv6        ///< Use the first IPv6 address found.
};
...
@interface SimplePingTrace : NSObject
...
@property (nonatomic, weak, readwrite, nullable) id<SimplePingTraceDelegate> delegate;

/*! Controls the IP address version used by the object.
 *  \details You should set this value before starting the object.
 */

@property (nonatomic, assign, readwrite) SimplePingTraceAddressStyle addressStyle;
...
@protocol SimplePingTraceDelegate <NSObject>
...
- (void)simplePing:(SimplePingTrace *)pinger didStartWithAddress:(NSData *)address;
...
- (void)simplePing:(SimplePingTrace *)pinger didFailWithError:(NSError *)error;
 ...
 *      considered API.
 *  \param pinger The object issuing the callback.
 *  \param packet The packet that was sent; this includes the ICMP header (`ICMPHeaderTrace`) and the
...
- (void)simplePing:(SimplePingTrace *)pinger didSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber;
...
- (void)simplePing:(SimplePingTrace *)pinger didFailToSendPacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber error:(NSError *)error;
 ...
 *  \param pinger The object issuing the callback.
 *  \param packet The packet received; this includes the ICMP header (`ICMPHeaderTrace`) and any data that
 *      follows that in the ICMP message but does not include any IP-level headers.
 *  \param sequenceNumber The ICMP sequence number of that packet.
 */

- (void)simplePing:(SimplePingTrace *)pinger didReceivePingResponsePacket:(NSData *)packet sequenceNumber:(uint16_t)sequenceNumber;
...
- (void)simplePing:(SimplePingTrace *)pinger didReceiveUnexpectedPacket:(NSData *)packet;
...
typedef struct ICMPHeaderTrace ICMPHeaderTrace;

__Check_Compile_Time(sizeof(ICMPHeaderTrace) == 8);
__Check_Compile_Time(offsetof(ICMPHeaderTrace, type) == 0);
__Check_Compile_Time(offsetof(ICMPHeaderTrace, code) == 1);
__Check_Compile_Time(offsetof(ICMPHeaderTrace, checksum) == 2);
__Check_Compile_Time(offsetof(ICMPHeaderTrace, identifier) == 4);
__Check_Compile_Time(offsetof(ICMPHeaderTrace, sequenceNumber) == 6);
